<template>
  <div id="app" style="visibility:hidden;" v-bind:style="{visibility:loading}"  >
     <div id="header">
        ...
        <h3 style="text-align: center;width:150px;margin:0 auto;">电影</h3>
    </div> 
    ...
    <div v-if="currentPageData" class="section"
     @touchmove="isShowLibrary" @touchend="hideLibrary" >
           ...
           <div class="passages" v-viewer="{movable: false}">
               <h4>数帧：</h4>
               <mt-swipe :auto="0"  style="height:150px;">
                  <mt-swipe-item v-for="item in currentPageData.excerpt">
                      <div style="display:flex;margin:5px 0px;background-color:#000;justify-content:center;">
                        <img :src="item" >
                      </div>
                  </mt-swipe-item>
               </mt-swipe>
...
export default {
                  ...
                  const t=setInterval(()=>{
                       if(that.libraryOffsetTop>that.priTop){
                        if(that.libraryOffsetTop-that.priTop>2){
                             that.libraryOffsetTop-=(that.libraryOffsetTop-that.priTop)/5;
                        }else{
                             that.libraryOffsetTop=that.priTop;
                        }
                       }else{
                          clearInterval(t);
                          this.preMove=null;
  ...
  created(){
    ...
    const that=this;
    axios.get(that.host+'/api/book_movie_music?type=movie')
